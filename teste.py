...
df = pd.read_excel(r'C:\Users\F8089999\OneDrive - TIM\_Consolidado Testes Pós Produção de '
                   r'Ofertas\Baixa de Faturas\Auto\Automações - Quality Assurance\12. Baixas - Dezembro.xlsx')
...
for index, row in df.iterrows():
    ...
    # Aguarda até o campo estar presente no DOM
    try:
        wait = WebDriverWait(browser, 30)  # Tempo máximo de espera de 30 segundos
        digitar_acesso = wait.until(EC.presence_of_element_located((By.NAME, "s_1_1_44_0")))
        digitar_acesso.send_keys(access_code)  # Digita o código de acesso no campo
        print("Código de acesso digitado com sucesso.")
    except Exception as e:
        print(f"Erro ao localizar ou interagir com o campo: {e}")
    ...
    # Loop para tratar alertas até que o fluxo continue
    while True:
        try:
            # Aguarda o alerta aparecer
            WebDriverWait(browser, 15).until(EC.alert_is_present())

            # Alterna para o alerta e pega o texto
            alerta = Alert(browser)
            alerta_texto = alerta.text
            print(f"Texto do alerta: {alerta_texto}")

            # Verifica se o alerta é o de "tripleta preenchida"
            if "Existe um atendimento Aberto com as mesmas características" in alerta_texto:
                print("Alerta de tripleta já preenchida detectado.")
                alerta.accept()  # Clica em "OK" no alerta
                # Clica novamente no botão "Prosseguir"
                try:
                    element_prosseguir = WebDriverWait(browser, 30).until(EC.element_to_be_clickable((By.XPATH, "//button[@data-display='Prosseguir']")))
                    element_prosseguir.click()
                    element_prosseguir.click()
                    element_prosseguir.click()
                except Exception as e:
                    print(f"Erro ao clicar novamente em 'Prosseguir': {e}")
            else:
                # Para outros alertas, apenas aceita
                alerta.accept()
                print("Outro alerta aceito.")
        except TimeoutException:
            print("Nenhum alerta presente ou fluxo concluído.")
    ...
    campo_upload.send_keys("C:/Users/F8089999/OneDrive - TIM/_Consolidado Testes Pós Produção de Ofertas/Baixa de Faturas/2024/De Acordo Dezembro/12. Re Pós Rollout Baixa Faturas - Linhas de teste - Dez24.msg")
